...
Class Action {
	...
	function login(){
			...
			extract($_POST);		
			$qry = $this->db->query("SELECT * FROM agents where agentname = '".$agentname."' and password = '".md5($password)."' ");
			if($qry->num_rows > 0){
	...
	function delete_agent(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM agents where id = ".$id);
		if($delete){
			return 1;
		}
	}
	function signup(){
		...
		$data .= ", agentname = '$email' ";
		$data .= ", password = '".md5($password)."' ";
		$chk = $this->db->query("SELECT * FROM agents where agentname = '$email' ")->num_rows;
		if($chk > 0){
			return 2;
			exit;
		}
		...
		if($save){
			...
			if($data){
				...
				$this->db->query("UPDATE agents set alumnus_id = $aid where id = $uid ");
				$login = $this->login2();
				if($login)
				return 1;
			}
	...
	function update_account(){
		...
		if(!empty($password))
		$data .= ", password = '".md5($password)."' ";
		$chk = $this->db->query("SELECT * FROM agents where agentname = '$email' and id != '{$_SESSION['login_id']}' ")->num_rows;
		if($chk > 0){
			return 2;
			exit;
		}
			$save = $this->db->query("UPDATE agents set $data where id = '{$_SESSION['login_id']}' ");
		if($save){
			$data = '';
			foreach($_POST as $k => $v){
				if($k =='password')
	...
	function save_city(){
		...
		$data = " name = '$name' ";
			if(empty($id)){
				$save = $this->db->query("INSERT INTO city set $data");
			}else{
				$save = $this->db->query("UPDATE city set $data where id = $id");
			}
		if($save)
			return 1;
	}
